...
visname =os.path.join(output_path,
                      'M31_20A_A_hex_HI.ms.contsub.regrid2kms')

# Split out the A config data, only
if not os.path.exists(visname):
    ...
    split(vis=full_visname,
          outputvis=visname,
          observation=myobs_aconfig,
          datacolumn='DATA')


imagepath = os.path.join(output_path, 'A_7pthex_imaging')
...
for i, setup_dict in enumerate(imaging_setups):
    ...
    for time_match in matched_time:
        ...
        imagename_run = os.path.join(imagepath,
                                    'M31_A_7pthex_{0}_{1}_robust{2}_taper_{3}_cell_{4}_channel_{5}'
                                    .format(confs,
                                            myweight,
                                            myrobust,
